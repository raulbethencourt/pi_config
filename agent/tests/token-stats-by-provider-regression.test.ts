...
describe("token-stats By Provider regression", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "token-stats-by-provider-"));
	});
	...
	it("groups provider prefixes dynamically in By Provider and leaves By Model unchanged", async () => {
		await createAnalyticsDb([
			{ timestamp: new Date().toISOString(), session_id: "r1", agent: "__main__", provider: null, model: "github-copilot/gpt-5.4-mini", cost_usd: 0.1 },
			{ timestamp: new Date().toISOString(), session_id: "r2", agent: "__main__", provider: null, model: "github-copilot/claude-sonnet-4.6", cost_usd: 0.2 },
		...
		const byModel = out.split("By Model").slice(1).join("By Model");

		expect(byProvider).toMatch(/github\s+2\s+\$0\.3000/);
		expect(byProvider).toMatch(/opencode\s+1\s+\$0\.3000/);
		expect(byProvider).not.toMatch(/orchestrator/);
		expect(byProvider).not.toMatch(/github-copilot\s+\d+/);

		expect(byModel).toMatch(/gpt-5\.4-mini\s+1\s+\$0\.1000/);
		expect(byModel).toMatch(/claude-sonnet-4\.6\s+1\s+\$0\.2000/);
		expect(byModel).toMatch(/deepseek-v4-flash-free\s+1\s+\$0\.3000/);
	});
});
