...
describe("token-stats orchestrator model identity", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "token-stats-orchestrator-model-"));
	});
	...
	it("shows the orchestrator identity instead of the github provider label for __main__ runs", async () => {
		await createAnalyticsDb([
			{
				timestamp: new Date().toISOString(),
				session_id: "orchestrator-model-run",
				agent: "__main__",
