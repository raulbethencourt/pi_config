...
describe("subagents default expansion", () => {
	...
	it("renders subagent blocks expanded by default", () => {
		const result = {
	...
	it("still supports collapsed rendering when explicitly disabled", () => {
		const result = {
			content: [{ type: "text", text: "done" }],
			details: {
				mode: "single",
		...
		expect(output).not.toContain("Task: Apply the fix");
		expect(output).not.toContain("edit(agent/extensions/subagents/index.ts)");
