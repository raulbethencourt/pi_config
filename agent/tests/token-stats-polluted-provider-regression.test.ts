...
describe("token-stats polluted provider regression", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "token-stats-polluted-provider-"));
	});
	...
	it("collapses polluted flat provider values back to the real provider family when recoverable, otherwise unknown", async () => {
		await createAnalyticsDb([
			{ timestamp: new Date().toISOString(), session_id: "r1", agent: "planner", provider: "gpt-5.4", model: "github-copilot/gpt-5.4", cost_usd: 0.1 },
			{ timestamp: new Date().toISOString(), session_id: "r2", agent: "planner", provider: "deepseek-v4", model: "opencode/deepseek-v4", cost_usd: 0.2 },
			{ timestamp: new Date().toISOString(), session_id: "r3", agent: "planner", provider: "flat-model-name", model: "flat-model-name", cost_usd: 0.3 },
		]);
		...
		expect(byProvider).toMatch(/opencode\s+1\s+\$0\.2000/);
		expect(byProvider).toMatch(/flat-model-name\s+1\s+\$0\.3000/);
		expect(byProvider).not.toMatch(/gpt-5\.4\s+1\s+\$0\.1000/);
